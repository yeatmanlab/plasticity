...
for ii = 2:size(params,1)
    ...
    plot([params(ii-1,2) params(ii,2)],[diff(prc(:,ii-1)) diff(prc(:,ii))],'-', 'color', c(ii,:));
    plot([params_scaled(ii-1,2) params_scaled(ii,2)],[diff(prc_scaled(:,ii-1)) diff(prc_scaled(:,ii))],'-', 'color', c(ii,:));
    if any(params(ii,2) == plotparams)
        plot(params(ii,2), diff(prc(:,ii)),'o', 'color', c(ii,:), 'markerfacecolor', c(ii,:));
        plot(params_scaled(ii,2), diff(prc_scaled(:,ii)),'o', 'color', [0 0 0], 'markerfacecolor', c(ii,:));
    end
...
for ii = 1:size(params,1)
    plot([params_scaled(ii,2) params_scaled(ii,2)],prc_scaled(:,ii),'-o', 'color', c(ii,:));
end
axis tight
set(gca, 'xtick',0:2:6);
grid('on')
...
for ii = 2:size(params,1)
    % Plot unscaled as solid line
    plot([params(ii-1,2) params(ii,2)],[rp(:,ii-1) rp(:,ii)],'-', 'color', c(ii,:));
    plot([params(ii-1,2) params(ii,2)],[rp_scaled(:,ii-1) rp_scaled(:,ii)],'-', 'color', c(ii,:));
    if any(params(ii,2) == plotparams)
